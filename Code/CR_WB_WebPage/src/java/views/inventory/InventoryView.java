 ...
 * @author csrm1
 */
@WebServlet(name = "InventoryView", urlPatterns = {"/InventoryView"})
public class InventoryView extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet InventoryView</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet InventoryView at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatcher = sc.getRequestDispatcher("/inventory/inventoryView.jsp");
        request.setAttribute("inventory", persistance.InventoryPersistance.getInstance().getObjectList().get(pos));
        ArrayList<WB_CR_INVENTORY_DETAIL> inventory_arr = new ArrayList<>();
        persistance.InventoryDetailPersistance.getInstance().getObjectList().forEach(x
                -> {
            if (x.getInventory_id().equals(id)) {
                inventory_arr.add(x);
            }
        });
        request.setAttribute("inventory_arr", inventory_arr);
