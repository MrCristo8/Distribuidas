...
public class InventoryServlet extends HttpServlet {
    ...
    {
        if (persistance.InventoryPersistance.getInstance().getObjectList().isEmpty())
        {
            try
            {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatcher = sc.getRequestDispatcher("/inventory/inventoryList.jsp");
        request.setAttribute("objList",
                persistance.InventoryPersistance.getInstance().getObjectList());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ServletContext sc = getServletContext();
        RequestDispatcher dispatcher = sc.getRequestDispatcher("/inventory/inventoryList.jsp");
        ArrayList<model.WB_CR_INVENTORY> filtered_list = new ArrayList<>();
        persistance.InventoryPersistance.getInstance().getObjectList().forEach(x
                ->
        {
            if (x.getMovement().getMovement_name().toUpperCase().contains(request.getParameter("search_string").toUpperCase()))
            {
                filtered_list.add(x);
            }
        });
